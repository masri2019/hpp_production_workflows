...
class CoverageDistribution:
    ...
    def fit(spectrum, tol = 1e-6, max_iters = 10000):
        ...
        for iteration in range(max_iters):
            ...
            # calculating the mean coverage of the extra component
            #numer = 0.0
            #denom = 0.0
            #weight_numer = 0.0
            #for cov in range( int(2 * cov_dist.cov_extra)):
            #    freq = spectrum[cov]
            ...
            #new_cov_extra = numer / denom

            # calculating the variation of the extra component
            #for cov in spectrum:
            #    freq = spectrum[cov]
            #    prob = assignment_extra[cov]
        ...
        for cov_value in sorted(spectrum)[1:]:
            if cov_value - 1 in spectrum:
                if spectrum[cov_value] > spectrum[cov_value - 1]:
                    has_increased = True
            if spectrum[cov_value] > (0.9 * max_spectrum) and has_increased:
